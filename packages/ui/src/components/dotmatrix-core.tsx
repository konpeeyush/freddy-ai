...
/*
 * The stylesheet is NOT imported here.
 *
 * Vendored from the @dotmatrix registry, which ships
 * `import "@workspace/ui/components/dotmatrix-loader.css"` on this line. The
 * widget renders inside a closed shadow root and adopts a single stylesheet
 * compiled from `apps/widget/src/styles.css`; a CSS import in the JS graph
 * emits a separate sheet for the host page's <head>, which the shadow tree
 * cannot see — so the loader came out unstyled while also shipping the CSS
 * twice. It is `@import`ed from that file instead.
 *
 * Re-running `shadcn add @dotmatrix/...` will restore the import. Remove it
 * again.
 */
/*
 * Two fixes to what the registry generated, both of which stop this compiling
 * here:
 *
 *   - The hooks are imported from `components/`, but `shadcn add` wrote them
 *     to `src/lib/`. This package maps `./lib/*` and `./components/*` to
 *     different directories, so the generated path resolved to nothing.
 *   - `ReactNode`, `useEffect` and `useRef` are imported and never used, which
 *     this repo compiles as an error.
 *
 * Re-running `shadcn add @dotmatrix/...` will restore both. Fix them again.
 */
import { useMemo } from "react";
import { useDotMatrixPhases, usePrefersReducedMotion, useCyclePhase } from "@workspace/ui/lib/dotmatrix-hooks";
