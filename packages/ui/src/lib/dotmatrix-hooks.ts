...
import type { DotMatrixPhase } from "@workspace/ui/components/dotmatrix-core";
...
export function useCyclePhase({ active, cycleMsBase, speed = 1 }: UseCyclePhaseOptions): number {
  ...
  useEffect(() => {
    /*
     * Vendor patch: the registry version called `setPhase(0)` here.
     *
     * Setting state synchronously in an effect costs a second render pass —
     * and here it was for nothing, because the return below already reports 0
     * whenever `active` is false. Reading it from `active` instead of storing
     * it keeps the same output with one render.
     *
     * Re-running `shadcn add @dotmatrix/...` will restore the call. Remove it.
     */
    if (!active) return
...
export function useSteppedCycle({
  ...
  useEffect(() => {
    if (!active) {
      ...
      /*
       * Vendor patch: the registry version also called `setStep(idleStep)`
       * here, which was pure cost — the return below already reports
       * `idleStep` whenever `active` is false, so the extra render changed
