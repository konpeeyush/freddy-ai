...
store.upsertPage("acme", errors, [
  chunk(errors, ["SSO"], "SSO_REDIRECT_MISMATCH means the callback url does not match."),
])
store.upsertPage("rival", rival, [
  chunk(rival, [], "Refunds are never issued under any circumstances."),
])

describe("tenant scoping", () => {
  test("keyword search never crosses a namespace", () => {
    const hits = store.searchKeyword("acme", "refunds", 50)
    expect(hits.length).toBeGreaterThan(0)
    expect(hits.every((hit) => hit.chunk.url.startsWith("https://acme.test"))).toBe(true)
  })

  test("vector search never crosses a namespace", () => {
    const hits = store.searchVector("acme", fakeEmbedding("refunds never issued"), 50)
    expect(hits.length).toBeGreaterThan(0)
    expect(hits.every((hit) => hit.chunk.url.startsWith("https://acme.test"))).toBe(true)
  })

  test("two namespaces can index the same url without colliding", () => {
    /*
    ...
    store.upsertPage("tenant-a", shared, [
      chunk(shared, ["Plans"], "Tenant A sees the enterprise plan."),
    ])
    store.upsertPage("tenant-b", shared, [
      chunk(shared, ["Plans"], "Tenant B sees the starter plan."),
    ])

    const a = store.searchKeyword("tenant-a", "plan", 50)
    const b = store.searchKeyword("tenant-b", "plan", 50)
  ...
  test("a page hash from one namespace is invisible to another", () => {
    expect(store.pageHash("acme", billing.url)).toBe(billing.hash)
    expect(store.pageHash("rival", billing.url)).toBeUndefined()
  })
})

describe("keyword retrieval", () => {
  test("finds an exact error code, which vectors are bad at", () => {
    const hits = store.searchKeyword("acme", "SSO_REDIRECT_MISMATCH", 10)
    expect(hits[0]?.chunk.headings).toEqual(["SSO"])
  })

  test("matches heading text the body never repeats", () => {
    const hits = store.searchKeyword("acme", "invoices", 10)
    expect(hits.some((hit) => hit.chunk.headings[0] === "Invoices")).toBe(true)
  })

  test("survives punctuation a visitor would actually type", () => {
    // FTS5 has its own query syntax; `?`, `*` and `(` are operators in it.
    // An unescaped question would be a syntax error rather than a search.
    const hits = store.searchKeyword("acme", "what's the refund policy? (30 days*)", 10)
    expect(hits.length).toBeGreaterThan(0)
  })
})

describe("ingest idempotence", () => {
  test("re-ingesting a page replaces its chunks rather than duplicating them", () => {
    const updated = page(billing.url, "Billing")
    updated.hash = "hash-updated"
    store.upsertPage("acme", updated, [
      chunk(updated, ["Refunds"], "Refunds now take 14 days."),
    ])

    const hits = store
      .searchKeyword("acme", "refunds", 50)
      .filter((hit) => hit.chunk.url === billing.url)
    expect(hits).toHaveLength(1)
    expect(hits[0]!.chunk.text).toContain("14 days")
  })

  test("pruning drops pages the crawl no longer sees and keeps the rest", () => {
    expect(store.removePagesNotIn("acme", [billing.url])).toBe(1)
    expect(store.pageHash("acme", billing.url)).toBe("hash-updated")
    expect(store.pageHash("acme", errors.url)).toBeUndefined()
  })
})

describe("clearing", () => {
  test("empties one namespace and leaves the others alone", () => {
    store.clear("acme")
    expect(store.stats("acme")).toHaveLength(0)
    expect(store.stats("rival")).toHaveLength(1)
  })
})
