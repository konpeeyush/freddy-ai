 ...
 *
 * Deliberately narrow: a first pass included `former` and `state`, which read
 * plausibly and dropped the Kyoto Imperial Palace ("Former ruling palace of
 * the Emperor of Japan") — one of the city's best-known sights. A landmark
 * being historical is the usual reason to visit it, so age words cannot be
 * the test.
 */
const NOT_VISITABLE =
...
export const findPlaces: ToolDefinition = {
  name: "findPlaces",
  description:
