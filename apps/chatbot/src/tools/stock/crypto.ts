 ...
 * Crypto prices.
 *
 * Live from CoinGecko's public endpoint — no key, CORS open. The one stock
 * tool whose data visibly changes between two calls a minute apart, which
 * makes it the honest way to check that a card is being re-rendered rather
 * than restored from a cached message.
 */
...
export const getCryptoPrice: ToolDefinition = {
  name: "getCryptoPrice",
  description:
