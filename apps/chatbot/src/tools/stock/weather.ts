 ...
 * Weather lookup.
 *
 * Open-Meteo needs no API key, which keeps this self-contained — one fewer
 * secret to hold and no rate-limit account to manage. It also sends
 * permissive CORS headers, which is what makes this tool portable to the
 * browser at all: most weather APIs cannot be called from a page, and would
 * have to stay server-side.
 *
...
/*
 * Hand-rolled in place of a Zod parse.
 *
 * This bundle ships to a customer's page, so a validator is weight it does
 * not need to carry. The obligation is the same either way: the response is
 * a third party's, so every field is checked before it reaches the card — a
 * missing `temperature_2m` must produce the "unexpected" message, not `NaN°C`.
 */
...
/** One line on the rest of the day, from the numbers already at hand. */
